...
exports.addGame = async (req, res, next) => {
    ...
    try {
        ...
        await SellerGames.findOneAndUpdate(
        ...
        await Seller.findOneAndUpdate({ name: sellerName },
            {
                $set: {
                    [`games.${gameName}`]: gameData.info.status
                }
            },
            { upsert: true });
...
exports.toggleAllItems = async (req, res, next) => {
    const { sellerName, status } = req.body;

    if (!sellerName || !status) {
        return res.status(400).json({
            message: 'Missing required data'
        });
    }
    try {
        ...
        // Update all games for the seller
        seller.games.forEach((value, key) => {
            seller.games.set(key, status);
        });
        await seller.save();

        // Update all storeInfo for games of the seller
        await SellerGames.updateMany(
            { storeName: sellerName },
            { $set: { 'storeInfo.info.status': status } },
            { upsert: true }
        );
